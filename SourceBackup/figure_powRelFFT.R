...
#==============================================================================#
data_file   <- "Build/model_powRelFFT.RData" # any RData inputs (or NA)
  ...
  summarize(across(where(is.numeric), 
                   list(mean = function(x) {mean(x, na.rm = T)},
                        ...
                        lower = function(x) {mean(x, na.rm = T) + 
                            qt(0.01, sum(!is.na(x)) - 1) * sd(x, na.rm = T) / sqrt(sum(!is.na(x)))},
                        upper = function(x) {mean(x, na.rm = T) + 
                            qt(0.99, sum(!is.na(x)) - 1) * sd(x, na.rm = T) / sqrt(sum(!is.na(x)))}))) %>% 
...
df.plot <- model.output %>% group_by(subgroup, freq) %>% 
  ...
  rename(GroupMean = mean.value,
         GroupCI1 = lower.ci.value,
         GroupCI2 = upper.ci.value)




df.plot$freq <- as.numeric(as.character(df.plot$freq))
...
p.pow <- df.plot %>%
  ...
  labs(y = expression(Relative~Power), x = "Frequency (Hz)")
...
p.gamma <- df.plot %>%
  ...
  theme(aspect.ratio = 1/1.3, panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), 
        ...
        legend.position = "none")+
  geom_segment(aes(x=55,xend=65,y=.05,yend=.05), lineend="butt", color="black", size=1) + 
  scale_x_continuous(limits = c(30,90), breaks = scales::pretty_breaks(n = 5)) +
  scale_y_continuous(limits = c(.045,.095), breaks = scales::pretty_breaks(n = 5)) +
  labs(y = expression(Relative~Power~x~Hz), x = "Frequency (Hz)")
...
p.output <-p.pow + 
  annotation_custom(
    ggplotGrob(p.gamma), 
    xmin = 10, xmax = 32, ymin=.015,ymax=.05)
p.output
