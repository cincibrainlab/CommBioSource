...
import.aac <- read_csv("Build/model_junAAC_68node.csv") %>% 
  mutate(z = FisherZ(value))

import.powpow <- read_csv("Build/model_powpowAAC_pearsons.csv")

import.powpow %>% fx.add.group_assignments(dat.clin) %>% 
  ggplot(aes(x=RSN, y=value, fill=group)) + geom_boxplot() +
  facet_grid(cols = vars(AACType)) +theme_minimal()
...
# CFC: ID, group, sex, CFC ("value") (nested within different nodes)
# TMS CSP: ID, group, sex, lower, upper, duration (upper-lower) ("value")
# Events: ID, group, sex, nodes, number_of_events ("value1"), duration_of_events ("value2")

# Statistical model; does a factor (group, sex) have an effect on the value 
...
df.aac <- df.aac.addZ %>% left_join(clininfo, by=c("eegid"="eegid")) %>% 
  ...
  mutate(subgroup = factor(subgroup, levels=c("TDC_M","FXS_M","TDC_F","FXS_F")),
         RSN = factor(RSN, levels=c("DMN","DAN","SAN","AUD","VIS","other")),
         upperband = factor(upperband, levels=c("gamma1","gamma2","epsilon")),
         lowerband = factor(lowerband, levels=c("theta","alpha1","alpha2"))) %>% 
...
fit.2 <- lme(value~group*lowerband*RSN, random = ~1|eegid,
             correlation=corCompSymm(form=~1|eegid), data=df.aac %>% 
               filter(powertype == "relative"))
anova(fit.2)
...
# least-squared means
emc <- emmeans(fit.final, ~ group*lowerband*RSN)
