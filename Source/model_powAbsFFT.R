...
dat.pow.absolute %>%  filter(as.numeric(freq) > 5)

# Data check
# dat.pow.absolute %>%  filter(eegid == "D01_108_rest") %>% group_by(freq) %>% 
#   summarize(value = mean(value)) %>%  ggplot(aes(x=freq,y=value)) + geom_line() +
#   xlim(0,80)

dat.pow.abs.long <- dat.pow.absolute %>% 
  fx.add.group_assignments(dat.clin) %>% 
  mutate(mgroup = factor(mgroup, levels=c("M4","M2","M1","F4","F3"),
