...
basename    = 'electrodePowerBandAverage'; % Edit
...
for i = 1 : length(power_results)
    ...
    for col_i = 5 : 7
        ...
        for row_i = 1 : size(res,1)
            powtable(global_count, :)= [power_results{i,1} power_col_labels{col_i} bandDefs{row_i,1} num2cell(res(row_i,:)) ];
            global_count = global_count + 1;
        end
    end
end

prepTable = cell2table(powtable,'VariableNames', [{'eegid'},{'measure'}, {'freq'}, chan_col_labels]);
%=========================================================================%
%                          EXPORT ENVIRONMENT                             %
%=========================================================================%

writetable(prepTable, target_file); 
% parquetwrite(target_file, prepTable );
%=========================================================================%
% RepMake           Reproducible Manuscript Toolkit with GNU Make          %     
%                  Version 8/2021                                         %
%                  cincibrainlab.com                                      %
